...
public class Sun : MonoBehaviour
{
    ...
    float latitude;
    float hourSun;
   public float minuteSun;
   public float monthSun;
   public float daySun;
    ...
    public void retrieveGlobalTime()
    {
        if (PlayerPrefs.HasKey("Hour") == true)
        {
            monthSun = PlayerPrefs.GetFloat("Month");
            daySun = PlayerPrefs.GetFloat("Day");
            hourSun = PlayerPrefs.GetFloat("Hour");
            minuteSun = PlayerPrefs.GetFloat("Minute");

        }
        else
        {
            this.hourSun = DateTime.Now.Hour;
            this.minuteSun = DateTime.Now.Minute;
            this.monthSun = DateTime.Now.Month;
            this.daySun = DateTime.Now.Day;
        }
        if ((server.gameObject.GetComponent("NamedPipesServerStream") as NamedPipesServerStream) != null)
        {
            time = new DateTime(2018, (int)monthSun, (int)daySun) + new TimeSpan((int)hourSun, (int)minuteSun, 0); //Active only when in normal operation
        }
        else
        {
            //time = new DateTime(2018, (int)monthSun, (int)daySun) + new TimeSpan((int)hourSun, (int)minuteSun, 0); //Active only when in data collection mode
        }
    }
